...
-- ─────────────────────────────────────────────────────────────────────────────
-- 3. Organisation type whitelist expansion
--    The Sprint 2 schema's chk_Organisations_OrgType allowed only
--      ('NGO','Student Group','CSR','Other')
--    but the Angular dropdown also offers 'Welfare Group' and
--    'Community Association'. Anything outside the CHECK list silently fails
--    the SP and the registration noops. Expand the whitelist so every option
--    the UI offers is accepted. Idempotent: drop the named constraint if
--    present, then re-create with the union.
-- ─────────────────────────────────────────────────────────────────────────────
IF EXISTS (
    SELECT 1 FROM sys.check_constraints
    WHERE name = 'chk_Organisations_OrgType'
      AND parent_object_id = OBJECT_ID('dbo.Organisations')
)
BEGIN
    ALTER TABLE dbo.Organisations DROP CONSTRAINT chk_Organisations_OrgType;
END;
GO

ALTER TABLE dbo.Organisations
    ADD CONSTRAINT chk_Organisations_OrgType
    CHECK (OrgType IN (
        'NGO',
        'Student Group',
        'CSR',
